...
Justo debajo de los KPIs principales hay una tarjeta con el % de Hl entregados sobre el total de Hl programados, por transportista (barra + porcentaje), para verlo de un vistazo sin bajar a la sección de detalle.
...
La sección de MR es ahora la primera del resumen operativo. La tarjeta "Tasa de rechazo vs. meta" muestra un solo gauge:
...
El indicador de volumen (Hl rechazado / Hl programado) ya no tiene su propio gauge en esta tarjeta, pero se sigue mostrando como número en la tarjeta "Universo del corte" de al lado (fila "Tasa de rechazo (volumen)").

Con archivos cargados, un cliente aporta a "Hl rechazado" según el mismo cruce de ambos archivos descrito arriba: si routes-data confirma el volumen rechazado se usa ese dato tal cual; si solo la Hoja de Ruta lo marca (por ejemplo porque routes-data no trae ese cliente en este corte), se usa `Hl_Programado - hl_fact` cuando `hl_fact` está presente, o el `Hl_Programado` completo si no. Este mismo criterio (por transportista) es el que alimenta el gráfico "Rechazos por transportista".

La tarjeta "Universo del corte", junto al gauge, muestra la **Tasa de rechazo (volumen)** como número explícito (Hl rechazado / Hl programado), junto a la tasa de contactos.

**Nota sobre los datos de ejemplo**: se corrigió una inconsistencia en el dataset de ejemplo (el que se ve antes de subir tus archivos, o si subes archivos pero no llegas a apretar "Procesar y actualizar"). El gauge "MR contactos" mostraba 0.46% mientras que "Tasa de rechazo (contactos)", en la misma pantalla, mostraba 6.5% — ambos números venían del mismo archivo fuente pero de dos pivotes distintos que no coincidían entre sí (uno de ellos con un periodo más largo que el corte del día). Ahora ambos indicadores usan la misma base y muestran el mismo número en todos lados (6.5% contactos, 3.0% volumen). Esto **no afecta tus archivos reales** — al subir tu Hoja de Ruta y routes-data y apretar "Procesar y actualizar", el tablero siempre calculó (y sigue calculando) MR contactos y MR volumen como una razón literal: total de clientes rechazados / total de clientes programados, y total de Hl rechazado / total de Hl programado, respectivamente.
...
1. Seguimiento de MR — arriba, la tasa de rechazo vs. meta (gauge "MR contactos") y el "Universo del corte" (que sí incluye contactos y volumen); debajo, un solo gráfico de barras combinado con contactos rechazados y Hl rechazados por transportista.
2. Detalle de rutas (tabla filtrable).
3. Riesgo y estado de ejecución / Rechazos registrados — según la fuente de datos, incluye la columna **Tipo** (Total / Parcial) cuando hay rechazos: "Parcial" cuando el pedido tuvo entrega y rechazo a la vez (o viene marcado así en la Hoja de Ruta / `status = PARTIAL_DELIVERY` en routes-data), "Total" cuando fue enteramente rechazado.
...
- La sección de "Riesgo y estado de ejecución" cambia de contenido según la fuente: con los datos de ejemplo muestra rutas marcadas manualmente en el Excel; con archivos subidos muestra los pedidos con rechazo registrado en la Hoja de Ruta (no hay un equivalente de "riesgo" en esos dos archivos), incluyendo si el rechazo fue total o parcial.
- Con los datos de ejemplo, los números de "Rechazos por transportista" (contactos y Hl) vienen de la propia ventana de MR del Excel original, que acumula un período más largo que el corte de rutas del día — por eso no van a cuadrar exactamente con el resto del tablero. Las tasas (%) sí están corregidas y son comparables. Con tus archivos cargados no aplica: todo se calcula del mismo corte.
